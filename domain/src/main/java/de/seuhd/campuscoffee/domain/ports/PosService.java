 ...
 * This interface defines the core business logic operations for managing Points
 * of Sale.
 * This is a port in the hexagonal architecture pattern, implemented by the
 * domain layer
 * and consumed by the API layer. It encapsulates business rules and
 * orchestrates
...
public interface PosService {
     ...
     *
     * @return a list of all POS entities; never null, but may be empty if no POSs
     *         exist
     */
    @NonNull
    List<Pos> getAll();
     ...
     * <li>Timestamps (createdAt, updatedAt) are managed by the
     * {@link PosDataService}.</li>
     * </ul>
     *
     ...
     * @throws PosNotFoundException      if attempting to update a POS that does not
     *                                   exist
     * @throws DuplicatePosNameException if a POS with the same name already exists
     */
    @NonNull
    Pos upsert(@NonNull Pos pos) throws PosNotFoundException, DuplicatePosNameException;

    /**
     * Imports a Point of Sale from an OpenStreetMap node.
     * Fetches POS data from OpenStreetMap using the {@link OsmDataService},
     * converts it to a POS entity,
     * and saves it to the system. If a POS with the same name already exists, it
     * will be updated.
     * <p>
     * The import process:
     * <ol>
     ...
     * @param campusType the campus type to assign to the imported POS; must not be
     *                   null
     * @return the created or updated POS entity; never null
     * @throws OsmNodeNotFoundException      if the OSM node with the given ID
     *                                       doesn't exist or cannot be fetched
     * @throws OsmNodeMissingFieldsException if the OSM node lacks required fields
     *                                       for creating a valid POS
     * @throws DuplicatePosNameException     if a POS with the same name already
     *                                       exists
     */
    @NonNull
    Pos importFromOsmNode(@NonNull Long nodeId, @NonNull CampusType campusType)
